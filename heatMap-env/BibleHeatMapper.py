"""
BibleHeatMapper.py
Original repo: https://github.com/UCLALibLab/text-heatmapper/blob/master/TextHeatmapper.py
Modified to accomodate large cleaned text blob
Sean O'Brien, September 5, 2020
usage: python BibleHeatmapper.py [document_file][labels_file]

Created using virtual enviorment with the following build:
boto            2.49.0
boto3           1.14.56
botocore        1.17.56
certifi         2020.6.20
...
This "freeze" will be provided as a requirements.txt in supporting docs.
"""
...
labels = [line.rstrip('\n').replace(' ', '_')
          for line in codecs.open(labelsfile, 'r', 'utf-8')]
...
if (os.path.isfile('LDAmodel.pkl') and (useCachedfiles == True)):
    ...
    model = gensim.models.ldamodel.LdaModel(
        corpus=dict_corpus, id2word=kt_dict, num_topics=100)
...
Z1 = dendrogram(Y, orientation="top", p=8,
...
im = ldamatrix.matshow(lda_sim, aspect='auto',
                       origin='lower', cmap=plt.get_cmap('YlGnBu'))
...
im = enmatrix.matshow(entropyMatrix, aspect='auto',
                      origin='lower', cmap=plt.get_cmap('YlGnBu_r'))
ticks = np.arange(0, nBundles, 10)
...
if (os.path.isfile('docVectors.pkl') and (useCachedfiles == True)):
    ...
    vectorizer = TfidfVectorizer(norm=None, use_idf=True, max_features=200000,
                                 min_df=.05, max_df=.95, ngram_range=(1, 3), tokenizer=tokenize)
...
im = axmatrix.matshow(dist, aspect='auto', origin='lower',
...
frame = pd.DataFrame(docs, index=[clusters], columns=[
...
for i in range(len(df)):
    ax.text(df.ix[i]['x'], df.ix[i]['y'], df.ix[i]
            ['title'], size=8, fontproperties=fontP)
...
print("BibleHeatmapper.py has completed")
